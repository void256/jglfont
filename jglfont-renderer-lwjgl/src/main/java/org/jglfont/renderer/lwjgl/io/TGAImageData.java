...
public class TGAImageData implements ImageData {
  ...
  public ByteBuffer loadImage(InputStream fis, boolean flipped, boolean forceAlpha, int[] transparent)
      throws IOException {
    return loadImageInternal(fis, flipped, forceAlpha, transparent, true);
  }

  public ByteBuffer loadMouseCursorImage(InputStream fis) throws IOException {
    return loadImageInternal(fis, true, false, null, false);
  }

  private ByteBuffer loadImageInternal(
      final InputStream fis,
      final boolean flippedParam,
      final boolean forceAlphaParam,
      final int[] transparent,
      final boolean forceNonePowerOfTwo) throws IOException {
    boolean forceAlpha = forceAlphaParam;
    if (transparent != null) {
      forceAlpha = true;
    }
    byte red = 0;
  ...
  public ByteBuffer getImageBufferData() {
    throw new TGAImageDataException("TGAImageData doesn't store it's image.");
  }
